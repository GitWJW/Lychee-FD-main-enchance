...
def _get_stepaudio_model_side_tokens() -> Tuple[int, int]:
    try:
        from vllm_model_stepaudio import StepAudioDuplexState
    except Exception as exc:
        raise RuntimeError(
            "StepAudio strict-native sampling requires "
            "vllm_model_stepaudio.StepAudioDuplexState to be importable."
        ) from exc

    stoken_token = getattr(StepAudioDuplexState, "out_stoken", None)
    control_token = getattr(StepAudioDuplexState, "out_control", None)
    if stoken_token is None or control_token is None:
        raise RuntimeError(
            "StepAudio strict-native sampling expected model-side sampled "
            f"tokens, got out_stoken={stoken_token}, out_control={control_token}, "
            f"diag_last={getattr(StepAudioDuplexState, 'diag_last', None)}."
        )
    return int(stoken_token), int(control_token)
...
def _sample_multihead_token_ids(
    ...
    for seq_group_idx, seq_group in enumerate(sampling_metadata.seq_groups):
        ...
        strict_model_side = bool(
            payload.get("stepaudio_require_model_side_tokens", False))
        ...
        if not row_indices:
            raise RuntimeError(
                "StepAudio strict-native sampling failed: empty sampler row "
                f"indices for seq_group_idx={seq_group_idx}, "
                f"sample_indices={seq_group.sample_indices}, parent_ids={parent_ids}, "
                f"logits_rows={logits.shape[0]}."
            )

        if strict_model_side:
            stoken_token, control_token = _get_stepaudio_model_side_tokens()
            stoken_token_ids[seq_group_idx] = [
                stoken_token for _ in row_indices
            ]
            control_token_ids[seq_group_idx] = [
                control_token for _ in row_indices
            ]
            has_stoken = True
            has_control = True
            continue
