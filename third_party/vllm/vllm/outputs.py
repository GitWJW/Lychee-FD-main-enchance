...
from typing import Any, Dict, Generic, List, MutableSequence, Optional
    ...
    def from_seq_group(
        ...
        if getattr(seq_group, "has_multihead_request_state", False):
            ...
            if mh_state is not None:
                # Prefer per-step deltas to avoid O(T^2) host-side copying of
                # side-token histories in streaming workloads.
                stoken_for_output = getattr(mh_state, "last_stoken_token_ids",
                                            None)
                if stoken_for_output is None:
                    stoken_for_output = getattr(mh_state, "stoken_input_ids",
                                                None)
                control_for_output = getattr(mh_state,
                                             "last_control_token_ids", None)
                if control_for_output is None:
                    control_for_output = getattr(mh_state,
                ...
                request_output = MultiHeadRequestOutput.from_request_output(
                    request_output,
                    stoken_token_ids=_copy_optional_int_list(stoken_for_output),
